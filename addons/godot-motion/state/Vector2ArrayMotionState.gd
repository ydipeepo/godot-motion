class_name Vector2ArrayMotionState extends MotionState
...
func validate() -> bool:
	if typeof(position) == TYPE_ARRAY:
		position = PoolVector2Array(position)
	if typeof(initial_position) == TYPE_ARRAY:
		initial_position = PoolVector2Array(initial_position)
	if typeof(final_position) == TYPE_ARRAY:
		final_position = PoolVector2Array(final_position)
	return (
		typeof(position) == TYPE_VECTOR2_ARRAY and
		typeof(initial_position) == TYPE_VECTOR2_ARRAY and
		typeof(final_position) == TYPE_VECTOR2_ARRAY and
