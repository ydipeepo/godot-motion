...
# プロセッサのリテンション期間
var processor_retention_duration: float:
	set(value):
		assert(0.0 <= value)
		_processor_retention_duration = value
	get:
...
func get_state(
	...
	target: Node,
	target_property: NodePath) -> MovadoState:
...
# バネアニメーションのビルダを作成します
func create_spring(preset = null) -> MovadoSpringBuilder:
	assert(preset == null or typeof(preset) == TYPE_INT)

	var builder := MovadoSpringBuilder.new(self)
	match preset:
		SPRING_STANDARD:
...
# 減衰アニメーションのビルダを作成します
func create_decay() -> MovadoDecayBuilder:
	return MovadoDecayBuilder.new(self)

# Tween アニメーションのビルダを作成します
func create_tween() -> MovadoTweenBuilder:
	return MovadoTweenBuilder.new(self)
...
func attach_processor(
	...
	target: Node,
	target_key: String,
	generator_init: MovadoGeneratorInit) -> void:
	...
	if not processor_exists:
		processor = MovadoProcessor.new()
		processor.name = _create_processor_name(target, target_key, processor_attach)
		processor.started.connect(_on_processor_started.bind(target, target_key, processor, processor_attach))
		processor.updated.connect(_on_processor_updated.bind(target, target_key, processor, processor_attach))
		processor.finished.connect(_on_processor_finished.bind(target, target_key, processor, processor_attach))
	...
	match processor_attach:
		PROCESSOR_ATTACH_PROPERTY:
			if (generator_init.initial_position == null or
				...
				if 0.0 < generator_init.delay:
					var position_binder := MovadoDelayedPosition.new(target, target_key)
			...
			if not processor_exists:
				processor.state = MovadoStateHelper.create(generator_init.initial_position)
		...
		PROCESSOR_ATTACH_METHOD, PROCESSOR_ATTACH_METHOD_DEFERRED:
			if not processor_exists:
				...
				processor.state = MovadoStateHelper.create(position)
	...
	processor.start(generator_init, _processor_retention_duration)
...
func compact() -> void:
	...
	while index > 0:
		index -= 1
		var processor := _container.get_child(index) as MovadoProcessor
...
var _container: Node
var _processor_retention_duration := 8.0
...
func _get_processor(
	object: Node,
	object_key: String,
	processor_attach: int) -> MovadoProcessor:
...
func _on_processor_started(
	...
	started.emit(target, target_key)
...
func _on_processor_updated(
	...
	updated.emit(target, target_key)
...
func _on_processor_finished(
	...
	finished.emit(target, target_key)
