class_name MovadoFloat32ArrayState extends MovadoState
...
func validate() -> bool:
	if typeof(position) == TYPE_ARRAY:
		position = PackedFloat32Array(position)
	if typeof(initial_position) == TYPE_ARRAY:
		initial_position = PackedFloat32Array(initial_position)
	if typeof(final_position) == TYPE_ARRAY:
		final_position = PackedFloat32Array(final_position)
	return (
		typeof(position) == TYPE_PACKED_FLOAT32_ARRAY and
		typeof(initial_position) == TYPE_PACKED_FLOAT32_ARRAY and
		typeof(final_position) == TYPE_PACKED_FLOAT32_ARRAY and
