class_name MovadoVector3ArrayState extends MovadoState
...
func validate() -> bool:
	if typeof(position) == TYPE_ARRAY:
		position = PackedVector3Array(position)
	if typeof(initial_position) == TYPE_ARRAY:
		initial_position = PackedVector3Array(initial_position)
	if typeof(final_position) == TYPE_ARRAY:
		final_position = PackedVector3Array(final_position)
	return (
		typeof(position) == TYPE_PACKED_VECTOR3_ARRAY and
		typeof(initial_position) == TYPE_PACKED_VECTOR3_ARRAY and
		typeof(final_position) == TYPE_PACKED_VECTOR3_ARRAY and
