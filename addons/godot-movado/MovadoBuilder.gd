...
func start(
	...
	final_position = null,
	initial_velocity = null) -> MovadoGeneratorInit:
	...
	get_context().attach_processor(
		_processor_update,
		0, # = MovadoContext.PROCESSOR_ATTACH_PROPERTY
		target,
		target_property,
		generator_init)
...
func start_method(
	...
	get_context().attach_processor(
		_processor_update,
		1, # = MovadoContext.PROCESSOR_ATTACH_METHOD
		target,
		target_method,
		generator_init)
	return generator_init
...
func start_method_deferred(
	...
	get_context().attach_processor(
		_processor_update,
		2, # = MovadoContext.PROCESSOR_ATTACH_METHOD_DEFERRED
		target,
		target_method,
		generator_init)
	return generator_init
...
var _processor_update := 0 # = MovadoContext.PROCESSOR_UPDATE_DEFAULT
